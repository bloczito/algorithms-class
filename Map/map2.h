...
std::pair<typename Map<K, V>::Iterator, bool> Map<K, V>::insert(const std::pair<K, V>& key_value) {
    ...
    BinaryNode<MapEntry<K, V>> *node = tree.find(me);
...
const V& Map<K, V>::operator[](const K& key) const {
    BinaryNode<MapEntry<K, V>> *node = tree.find(MapEntry<K, V>(key, V()));
...
V& Map<K, V>::operator[](const K& key) {
    ...
    BinaryNode<MapEntry<K, V>> *node = tree.find(dummy);
...
typename Map<K, V>::Iterator Map<K, V>::remove(const K &key) {
    MapEntry<K, V> dummy(key, V());
    Node<MapEntry<K, V>> node = tree.find(dummy);
    Iterator result = end();
    if (node) {
