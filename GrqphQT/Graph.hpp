...
// zwraca id wierzchołka koncowego
    ...
    public:
        explicit DFSIterator(Graph<V, E> &graph, size_t start_id = 0);
        bool operator==(const DFSIterator& dfsi) const { return index == dfsi.index; }
        bool operator!=(const DFSIterator& dfsi) const { return index != dfsi.index; }
        DFSIterator& operator++();
        DFSIterator operator++(int) {DFSIterator it(*this); ++(*this); return it; }
        V& operator*() const { index = vertexStack.top(); return graph._vertices[index]; }
        V* operator->() const { index = vertexStack.top(); return &graph._vertices[index]; }
        operator bool() const { return index < graph._vertices.size(); }
    };
    ...
    public:
        BFSIterator(Graph<V, E> &graph, size_t start_id = 0);
        bool operator==(const BFSIterator& dfsi) const { return index == dfsi.index; }
        bool operator!=(const BFSIterator& dfsi) const { return index != dfsi.index; }
        BFSIterator& operator++();
        BFSIterator operator++(int) {BFSIterator it(*this); ++(*this); return it; }
        V& operator*() const { index = vertexQueue.front(); return graph._vertices[index]; }
        V* operator->() const { index = vertexQueue.front(); return graph._vertices[index]; }
        operator bool() const {  return index < graph._vertices.size(); }
    };
...
typename Graph<V, E>::DFSIterator Graph<V, E>::beginDFS(std::size_t vertex_id) {
    return Graph::DFSIterator(*this, vertex_id);
}

template<typename V, typename E>
typename Graph<V, E>::DFSIterator Graph<V, E>::endDFS() {
    return Graph::DFSIterator(*this, _vertices.size());
}
...
typename Graph<V, E>::BFSIterator Graph<V, E>::endBFS() {
    return Graph::BFSIterator(*this, _vertices.size());
}
...
template<typename V, typename E>
Graph<V, E>::DFSIterator::DFSIterator(Graph<V, E> &graph, std::size_t start_id)
    : graph(graph), index(start_id) {
...
typename Graph<V, E>::DFSIterator &Graph<V, E>::DFSIterator::operator++() {
    ...
    for (int i = 0; i < graph._vertices.size(); ++i) {
        if (graph._nbMatrix[index][i] && visited[i] == false) {
            vertexStack.push(i);
            visited[i] = true;
        }
    }
...
template<typename V, typename E>
Graph<V, E>::BFSIterator::BFSIterator(Graph<V, E> &graph, size_t start_id)
    : graph(graph), index(start_id) {

        visited.resize(graph._vertices.size());
