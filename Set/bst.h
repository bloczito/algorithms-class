...
//        typename BST<T>::Iterator Iter;
        ...
        explicit Iterator(const Node* node, Node *root);
    ...
    const Node* find_It_Helper(const T& x ) const;
	T* find(const T& x);
...
template<class T>
const BinaryNode<T> *BST<T>::find_It_Helper(const T &x) const {
    return find(x, root);
}
...
//================= KLASA ITERATOR ================
template<class T>
BST<T>::Iterator::Iterator(const BST<T>::Node *node, BST<T>::Node *root) : current(nullptr)
{
//    Node * tmp = root;
    if(node == nullptr)
        return;
...
template<class T>
void BST<T>::Iterator::fillStack(BST::Node *node) {
    while ( node != nullptr) {
        stack_.push(node);
        node = node->left;
    }
